...

{
	...
	if (!CryptAcquireContext(&hCryptProv, pKeyContainer, pServiceProvider, PROV_RSA_FULL, CRYPT_NEWKEYSET))
	{
		if (!CryptAcquireContext(&hCryptProv, pKeyContainer, pServiceProvider, PROV_RSA_FULL, 0))
		{
			return -2;
		}
	}
	...
	// Hash the password.
	if (!CryptHashData(hHash, (BYTE *)pDataForHash, strlen(pDataForHash), 0))
	{
	...
	if (!CryptEncrypt(hKey, 0, TRUE, CRYPT_OAEP, NULL, &lCryptedBufferLength, 0))
	{
...
long Encrypt::encrypt(const char *pBufferToEncrypt, char *pCryptedBuffer, unsigned long ulCryptedBufferLength)

{
	...
	if (pBufferToEncrypt == (const char *)NULL || pCryptedBuffer == (char *)NULL || ulCryptedBufferLength < lCryptedDataLength)
		return -1;
...

	{
		...
		if (!CryptEncrypt(hKey, 0, TRUE, CRYPT_OAEP, (BYTE *)pLocalCryptedBuffer, &lCryptedDataLength, ulCryptedBufferLength))
		{
		...
		{
			sprintf(pPointerToCryptedBuffer, "%4ld", (long)(pLocalCryptedBuffer[lCryptedIndex]));
...
long Encrypt::decrypt(const char *pCryptedBuffer, char *pDecryptedBuffer, unsigned long uDecryptedBufferLength)

{
	...
	if (pCryptedBuffer == (const char *)NULL || pDecryptedBuffer == (char *)NULL)
		return -1;
	...
	if (!CryptDecrypt(hKey, 0, TRUE, CRYPT_OAEP, (BYTE *)pDecryptedBuffer, &lDecryptedDataLength))
	{
		CryptDestroyKey(hKey);
		CryptDestroyHash(hHash);
		CryptReleaseContext(hCryptProv, 0);
...

{
	...
	while ((strrchr(sBuffer, 9) != NULL) || (strrchr(sBuffer, 10) != NULL) || (strrchr(sBuffer, 13) != NULL))
	{
...
long clsCryptography::lGetBufferLength() { return lBufLen; }

// 99% of the encryption / decryption code is the same
// so keep in this common function
bool clsCryptography::bEncryptDecrypt(char *sData, char *sPassword, bool bEncrypt)

{
	...
	// Hash the password.
	if (!CryptHashData(hHash, (CONST BYTE *)sPassword, strlen(sPassword), 0))
	{
	...
	{
		// Encrypt data.
		if (!CryptEncrypt(hKey, 0, 1, 0, (BYTE *)sBuffer, &lLength, lBufLen))
		{
...

{
	...
	for (int i = 0; i < 8; i++)
	{
		lResult = (10 * lResult) + (sEncrypted[i] - sNumberEncryptString[i]);
	}
...

{
	...
	if ((pLocalCryptedBuffer = new char[lLocalCryptedBufferLength + 1]) == (char *)NULL)
		return 1;
	...
	{
		sprintf(pPointerToCryptedBuffer, "%4ld", (long)(pLocalCryptedBuffer[lIndex]));
		pPointerToCryptedBuffer += 4;
	}
...

{
	...
	if ((pLocalCryptedBuffer = new char[(lCryptedBufferLength / 4) + 1]) == (char *)NULL)
		return 1;
...
long Encrypt::getCryptedBufferLength(const char *pBufferToEncrypt) { return strlen(pBufferToEncrypt) * 2; }
...
long Encrypt::encrypt(const char *pBufferToEncrypt, char *pCryptedBuffer, unsigned long ulCryptedBufferLength)
{
	...
	int indexForCrypted;
	for (indexForToEncrypt = 0, indexForCrypted = 0; indexForToEncrypt < ulBufferToEncryptLength; indexForToEncrypt++, indexForCrypted += 2)
	{
		if ((int)(pBufferToEncrypt[indexForToEncrypt] + 3) >= 32 &&
			(int)(pBufferToEncrypt[indexForToEncrypt] + 3) <= 126
			// chars creating problems in URL...
			&& (char)(pBufferToEncrypt[indexForToEncrypt] + 3) != '<' && (char)(pBufferToEncrypt[indexForToEncrypt] + 3) != '>' &&
...
long Encrypt::decrypt(const char *pCryptedBuffer, char *pDecryptedBuffer, unsigned long uDecryptedBufferLength)
{
	...
	for (indexForCrypted = 0, indexForDecrypted = 0; indexForCrypted < uCryptedBufferLength; indexForCrypted += 2, indexForDecrypted++)
	{
		if (pCryptedBuffer[indexForCrypted] == '1')
			pDecryptedBuffer[indexForDecrypted] =
				pCryptedBuffer[indexForCrypted + 1] - 3; // the key for encryption is 3 that is subtracted to ASCII value
		else
...
string Encrypt::encrypt(string stringToBeEncrypted)
{
	...
	if (Encrypt::encrypt(stringToBeEncrypted.c_str(), pBufferCrypted, lBufferCryptedLength + 1) != 0)
	{
...
string Encrypt::decrypt(string stringToBeDecrypted)
{
	...
	if (Encrypt::decrypt(stringToBeDecrypted.c_str(), pDecryptedBuffer, lDecryptedBufferLength + 1) != 0)
	{
...
int Encrypt::convertFromBase64ToBinary(const char *b64message, unsigned char **buffer, size_t *length)
{
	...
	*length = BIO_read(bio, *buffer, strlen(b64message));
	if (*length != decodeLen)
	{
		...
		throw runtime_error(string("check length failed") + ", *length: " + to_string(*length) + ", decodeLen: " + to_string(decodeLen));
	}

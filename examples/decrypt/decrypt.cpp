...

{
	...
	if (iArgc != 2)
	{
		std::cerr << "Usage: " << pArgv[0] << " <string to decrypt>" << std::endl;
	...
	// for (long lIndex = 0; lIndex < 10000; lIndex++)
	{
		...
		{
			std::cerr << "Encrypt:: getDecryptedBufferLength failed" << endl;

			return 1;
		}

		if ((pDecryptedBuffer = new char[lDecryptedBufferLength + 1]) == (char *)NULL)
		{
			std::cerr << "new failed";
		...
		if (Encrypt::decrypt(pArgv[1], pDecryptedBuffer, lDecryptedBufferLength + 1) != 0)
		{
			std::cerr << "Encrypt:: decrypt failed" << endl;

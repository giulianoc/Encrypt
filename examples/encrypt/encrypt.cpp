...

{
	...
	if (iArgc != 2)
	{
		std::cerr << "Usage: " << pArgv[0] << " <string to encrypt>" << std::endl;
	...
	// for (long lIndex = 0; lIndex < 10000; lIndex++)
	{
		...
		if ((pBufferCrypted = new char[lBufferCryptedLength + 1]) == (char *)NULL)
		{
			std::cerr << "new failed";
		...
		if (Encrypt::encrypt(pArgv[1], pBufferCrypted, lBufferCryptedLength + 1) != 0)
		{
